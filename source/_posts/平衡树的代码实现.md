...
{% tip warning %}
<div class="p center">维护两个语言的代码不太方便，可能会有语义不一样的地方。如果出现语义不同的地方，以<code>Kotlin</code>的实现为准。</div>
{% endtip %}
...
&emsp;&emsp;其中前三条都很好理解，第四条是为了方便判断树的重心方向，这个后面会具体说明。
...
&emsp;&emsp;现在我们遇到了一个问题：如何快速判断树的重心？我们不可能每次判断重心的时候都把子树遍历一遍，这样子效率太低了。所以我们选择存储树中节点的数量，这样子在判断重心的时候只需要比较左右子树的节点数量即可。
...
class Node<T : Comparable<T>>(value: T) : Comparable<T> {
    ...
    fun getLeftSonAmount() = if (notLeft()) 0 else left!!.sonAmount
    fun getRightSonAmount() = if (notRight()) 0 else right!!.sonAmount
    ...
    fun getBalance(): BalanceDirection {
        val dif = getLeftSonAmount() - getRightSonAmount()
        if (dif < 0) return RIGHT
        if (dif == 0) return BalanceDirection.CENTER
        return LEFT
    }
    ...
    internal fun repairDeep() {
        ...
        while (point != null) {
            point = point.apply {
                deep = max(getLeftDeep(), getRightDeep()) + 1
                sonAmount = getLeftSonAmount() + getRightSonAmount() + 1
                father
            }
        }
    }
...
struct Node {
    ...
    BalanceDirection getBalance() const {
        int dif = getLeftSonAmount() - getRightSonAmount();
        if (dif < 0) return RIGHT;
    ...
    void repairDeep() {
        Node* point = this;
        do {
            point->deep = max(point->getLeftDeep(), point->getRightDeep()) + 1;
            point->sonAmount = point->getLeftSonAmount() + point->getRightSonAmount() + 1;
            point = point->father;
        } while (point != nullptr);
    }
    ...
    /** 获取左子树子节点数量，左子树不存在返回0 */
    inline int getLeftSonAmount() const {
        return notLeft() ? 0 : left->sonAmount;
    }
    
    /** 获取右子树子节点数量，右子树不存在返回0 */
    inline int getRightSonAmount() const {
        return notRight() ? 0 : right->sonAmount;
    }
